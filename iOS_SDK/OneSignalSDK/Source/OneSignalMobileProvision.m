...
+ (void)logInvalidMobileProvisionError:(NSString *)message {
    [OneSignalSmart onesignal_Log:ONE_S_LL_ERROR message:message];
}
...
+ (UIApplicationReleaseMode) releaseMode {
    ...
    if (mobileProvision) {
        [OneSignalSmart onesignal_Log:ONE_S_LL_DEBUG message:[NSString stringWithFormat:@"mobileProvision: %@", mobileProvision]];
        entitlements = [mobileProvision objectForKey:@"Entitlements"];
    }
    else
        [OneSignalSmart onesignal_Log:ONE_S_LL_DEBUG message:@"mobileProvision not found"];
